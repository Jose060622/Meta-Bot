...
emoji_bot = "♨️💫"
 ...
 return `
 *🏓_•*Respondido en: 0.00${speedB} Segundos*
 *⏳_•Runtime: ${temporizado(uptime)}*
 
 🚀 - *Velocidad y tiempo online*
 `
 }
 
 exports.erroB = (comando, q) => {
 return `[📍] Cᴏᴍᴀɴᴅᴏ : *${comando}*\n[❗] Mᴜsɪᴄᴀ : *${q}*\n[‼️] Sɪᴛᴜᴀcion : *Error!* \n*『Intente usar otro play』*`
 }
 ...
 き⃟🏷️Cᴏᴍᴀɴᴅᴏ : *${comando}*
 き⃟🏷 Sɪᴛᴜᴀcion : *Error!* 
 *『Intente usar de nuevo』*`  
 }
 
 exports.tterro = () => {
 return `debe usarse asi き⃟${p}ppt piedra, き⃟${p}ppt papel ou き⃟${p}ppt tijera`
 }
 
 exports.ban = (NomeDoBot) => {
 return`${emoji_bot} Usted esta baneado del bot`
 }
 
 exports.ownerB = () => {
 return`${emoji_bot} Este comando solo lo puede usar mi dueño`
 }
 
 exports.playErro = () => {
return `*_🗣¡El comando es erróneo! ❌ _ * \n * _🗣 Prueba ${p} play1 ¡Para reenviar música! ✔️__*`
}
...
exports.sexo = () => {
return "Aquí no hay prácticamente nada, solo se usa para engañar a los muggles 🤭"
}

 exports.Lv = () => {
 return`${emoji_bot} Link invalido`
 }
 ...
 exports.admin = () => {
 return`${emoji_bot} Este comando solo lo pueden usar los adms`
 }
 
 exports.Badmin = () => {
 return`${emoji_bot} Este comando solo lo puede usar el bot o admins del grupo`
 }
 
 exports.AdmDoBot = () => {
 return`${emoji_bot} Este comando solo se puede usar cuando el bot es admin del grupo`
 }
 
 //limit
 exports.cash_adquirir = (pushname, p) => {
  return`
 *き⃟🐋Disculpe ${pushname} su dinero se acabo ...*
 *き⃟🐋Para consegir mas dinero compre un pase de nivel*
 
 き⃟🐋 - Para comprar dinero : *き⃟${p}buycash 10*
 き⃟🐋 - En caso que tenga deudas hable con el creador ...
 き⃟🐋+ ${SeuNome} - https://wa.me/${OwnerNumber}`
 }
 
 exports.limitend = (pushname) => {
  return`*Disculpe me ${pushname} su limite se acabo*\n*el limite se restablece cada 24:00*`
 }
